...
const Navbar: React.FC<NavbarProps> = ({
  ...
  const handleLoginClick = () => {
    ...
    setTimeout(() => {
      window.location.href = 'https://shah-sultan-s-ielts-academy.web.app/student/login';
  ...
  return <header className="bg-white text-primary shadow-md sticky top-0 z-50">
      <div className="container py-4">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            {isLoggedIn ? <div className="relative" ref={profileDropdownRef}>
                ...
                {isProfileDropdownOpen && <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-10">
                    {userRole === 'student' && (
                      <>
                        <Link to="/dashboard/profile" className="block px-4 py-2 text-sm text-text-primary hover:bg-secondary">
                          Profile
                        </Link>
                        <Link to="/dashboard" className="block px-4 py-2 text-sm text-text-primary hover:bg-secondary">
                          Dashboard
                        </Link>
                      </>
                    )}
                ...
                <button 
                  ...
                  className="btn-outline text-sm py-2 px-4 rounded-md border border-primary text-primary hover:bg-primary hover:text-white focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-1 focus:ring-offset-white transition-colors"
                ...
                <button 
                  ...
                  className="btn-primary text-sm py-2 px-4 rounded-md bg-accent text-white hover:bg-accent-light focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-1 focus:ring-offset-white transition-colors"
                  data-testid="header-enroll-button"
                >
                  Enroll Now
                </button>
              </div>}
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="p-4 border-t border-gray-200">
              {isLoggedIn ? <div className="space-y-2">
                  ...
                  <button 
                    onClick={handleLoginClick}
                    className="block w-full text-center py-2 px-4 border border-primary text-primary rounded-md hover:bg-primary hover:text-white transition-colors"
                  ...
                  <button 
                    onClick={() => openEnrollmentModal()}
                    className="block w-full text-center py-2 px-4 bg-accent text-white rounded-md hover:bg-accent-light transition-colors"
                    data-testid="mobile-enroll-button"
                  >
                    Enroll Now
                  </button>
                </div>}
        ...
        {/* Login Loading Animation Modal */}
        {showLoginAnimation && (
          <div className="fixed inset-0 bg-black bg-opacity-70 z-[100] flex items-center justify-center">
            <div className="bg-white rounded-2xl p-8 shadow-2xl max-w-md w-full mx-4 flex flex-col items-center">
