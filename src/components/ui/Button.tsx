...
const Button: React.FC<ButtonProps> = ({
  ...
  if (href) {
    return <a href={href} className={buttonClasses} target={target} rel={rel} aria-label={ariaLabel} data-testid={dataTestId}>
        {children}
      </a>;
  }
  if (to) {
    return <Link to={to} className={buttonClasses} aria-label={ariaLabel} data-testid={dataTestId}>
        {children}
      </Link>;
  }
  return <button type={type} className={buttonClasses} disabled={disabled} onClick={onClick} aria-label={ariaLabel} data-testid={dataTestId}>
      {children}
    </button>;
};
